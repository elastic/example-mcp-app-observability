...
export const apmHealthSummaryFixtures: FixtureSet = {
  degraded: fixture("Degraded cluster", {
    ...
    alerts: {
      ...
      recovered_count: 38,
      top_rules: [
        { name: "[K8s OTel] Container memory near limit", count: 6 },
        { name: "[K8s OTel] Pod CrashLoopBackOff", count: 3 },
        { name: "checkout p99 > 800ms", count: 2 },
        { name: "[K8s OTel] OOMKilled containers", count: 2 },
        { name: "frontend error rate > 2%", count: 1 },
      ],
      active_samples: [
        {
          rule: "[K8s OTel] Container memory near limit",
          reason: "Average metrics.k8s.pod.memory.working_set is 412,065,344, above the threshold of 393,216,000 (duration: 5 mins, data view: metrics-*)",
          instance_id: "checkout-7b8c9d-x2k9m",
          severity: "warning",
          started_ms: now - 22 * 60 * 1000,
        },
        {
          rule: "checkout p99 > 800ms",
          reason: "p99 transaction.duration.us for service.name=checkout is 921ms, above 800ms threshold",
          instance_id: "checkout",
          severity: "critical",
          started_ms: now - 9 * 60 * 1000,
        },
        {
          rule: "[K8s OTel] Pod CrashLoopBackOff",
          reason: "Pod ad-6dd68bf547-rrw4j has been in CrashLoopBackOff state for 3 mins",
          instance_id: "ad-6dd68bf547-rrw4j",
          severity: "critical",
          started_ms: now - 3 * 60 * 1000,
        },
      ],
    },
    slos: {
      ...
      top_violations: [
        {
          ...
          one_hour_burn_rate: 28.3,
          one_day_burn_rate: 14.1,
          indicator_type: "sli.kql.custom",
          last_evaluated_ms: now - 30 * 1000,
        },
        {
          name: "checkout · p95 latency < 500ms",
          sli_value: 0.842,
          target: 0.95,
          error_budget_remaining: -2.16,
          one_hour_burn_rate: 7.2,
          one_day_burn_rate: 3.4,
          indicator_type: "sli.kql.custom",
          last_evaluated_ms: now - 30 * 1000,
        },
        {
          name: "[Kubernetes OTel] Deployment Replica Availability 99.5% Rolling 30 Days",
          sli_value: 0.9837,
          target: 0.995,
          error_budget_remaining: -2.26,
          one_hour_burn_rate: 5.1,
          one_day_burn_rate: 2.0,
          indicator_type: "sli.metric.timeslice",
          last_evaluated_ms: now - 60 * 1000,
        },
        {
          name: "frontend · availability",
          sli_value: 0.9889,
          target: 0.99,
          error_budget_remaining: -0.11,
          one_hour_burn_rate: 1.3,
          one_day_burn_rate: 0.8,
          indicator_type: "sli.kql.custom",
          last_evaluated_ms: now - 30 * 1000,
        },
      ],
  ...
  healthy: fixture("Healthy", {
    ...
    alerts: { active_count: 0, recovered_count: 4, top_rules: [], active_samples: [] },
    slos: {
      configured: true,
      violated_count: 0,
      healthy_count: 16,
